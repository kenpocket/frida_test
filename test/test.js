...
var gr_id, gr_url, gr_imgUrl, gr_imgName, gr_texNums, gr_info, dl_btn, dl_title, dl_status, timeID, objNums,
    objNamePext = {}, mtl = "", previewName = "预览图.jpeg", linkFileName = "快捷方式.url", imgNamePextList = {};

var _disPlayMod = 1;
var mid, url, freedl, list, favList = {}, favType = {}, webUrl = location.href, webSite, colNums, colTop,
    searchPage = 1;
...
IconIndex=1`;
    ...
    var PackAll = function () {
        ...
        folder.generateAsync({type: "blob"}).then(content => {
            ...
            gr_info.innerHTML = '<span style="color: #08f508; text-shadow: 0 0 3px #000000;">文件大小：' + (content.size / 1024 / 1024).toFixed(1) + ' MB</span>';
        });
        ...
        window.top.document.getElementById('gr_iframe').contentWindow.postMessage({
            "postType": "setDownLoad",
            "postData": {"member": _ACCOUNT, "mid": mid}
        }, '*');
    }
    ...
    window.drawhookcanvas = function (e, imagemodel) {
        ...
        if (imagemodel) {
            ...
            imagemodel.attributes.images.forEach(function (img) {
                let alpha_is_check = alpha == "A" ? img.options.format == alpha : true;
...
\
    ...
    window.onload = function () {
        ...
        $('body').on('click', '#gr_setdl', function () {
            gr_iframe.contentWindow.postMessage({
                "postType": "setDownLoad",
                "postData": {"member": _ACCOUNT, "mid": mid}
            }, '*');
        });
        ...
        $('body').on('click', '.bootstrap-tagsinput .tag', function () {
            ...
            if (mid) {
                var data = {
                    "postType": "setData",
                    "postData": {"member": _ACCOUNT, "mid": mid, "url": url, "freedl": freedl, "type": type}
                };
                gr_iframe.contentWindow.postMessage(data, '*');
            } else {
                alert('DOM change!');
